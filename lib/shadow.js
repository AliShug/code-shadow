...
export default class Shadow {
  ...
  constructor(properties) {
    ...
    let textContent = document.createTextNode(`${this.description[0]} `);
    let emContent = document.createElement('em');
    if (this.description.length > 1) {
      emContent.appendChild(document.createTextNode(this.description[1]));
    }
    ...
    this.textElement.appendChild(emContent);
    this.parentElement.appendChild(this.textElement);
    // Indent
    let indent = (this.depth + 1)*6;
    ...
    // Interaction
    let hoverEvent = () => {
      this.lineElement.classList.add('highlight');
      this.textElement.classList.add('highlight');
    };
    let deHoverEvent = () => {
      this.lineElement.classList.remove('highlight');
      this.textElement.classList.remove('highlight');
    };
    this.textElement.addEventListener('click', () => {
      this.editor.setCursorBufferPosition([this.startLine-1, 9999]);
    });
    this.textElement.addEventListener('mouseenter', hoverEvent);
    // this.lineElement.addEventListener('mouseenter', hoverEvent);
    this.textElement.addEventListener('mouseleave', deHoverEvent);
    // this.lineElement.addEventListener('mouseleave', deHoverEvent);
  }
  ...
  update(lineHeight, scrollOffset, workingOffset) {
    ...
    let desiredTop = (screenStart.row - 1) * lineHeight - scrollOffset;
    ...
    let textTop = Math.min(Math.max(top + Math.round(lineHeight/8), 0), desiredTop + height - lineHeight);
