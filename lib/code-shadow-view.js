...
export default class CodeShadowView {
  ...
  constructor(editor) {
    ...
    this.paneInitialSize = 0;
    this.resizeStartPoint = 0;
    let editorElement = atom.views.getView(editor);
    editorElement.setAttribute('with-codeshadow', '');
    // Structural changes
  ...
  // Shadow Pane resizing logic
  resizeStarted({pageX, which}) {
    this.resizeStartPoint = pageX;
    console.log("Resize start " + pageX);
    this.paneInitialSize = this.parentElement.offsetWidth;
    console.log("Initial " + this.paneInitialSize);

    let resizeStopped = () => {
      document.removeEventListener('mousemove', resizeView);
      document.removeEventListener('mouseup', resizeStopped);
    };

    let resizeView = ({pageX, which}) => {
      if (which !== 1) return resizeStopped();

      let deltaX = pageX - this.resizeStartPoint;
      console.log(deltaX);
      console.log("from " + this.resizeStartPoint);
      let width = this.paneInitialSize - deltaX;
      // Parent element is the code shadow pane
      // Need to trigger an update on the adjacent pane to get the scrollbar
      // to move...
      let editorPane = atom.workspace.getActivePane().element;
      this.parentElement.style['flex-basis'] = `${width}px`;
      editorPane.style['flex-basis'] = editorPane.offsetWidth;
      editorPane.style['flex-basis'] = '';
    };

    document.addEventListener('mousemove', resizeView);
  ...
  shade() {
    ...
    if (this.parentElement === null) {
      // Create right-hand panel element if it doesn't exist
      let editorElement = atom.views.getView(this.editor);
      let scrollView = editorElement.querySelector('.scroll-view');
      let paneElement = document.createElement('code-shadow-pane');
      let resizeHandle = document.createElement('code-shadow-resize-handle');
      resizeHandle.addEventListener('mousedown', (e) => this.resizeStarted(e));
      editorElement.appendChild(paneElement);
      paneElement.appendChild(resizeHandle);
      this.parentElement = paneElement;
    }
